...
A few effects exist on the raster path but cannot be expressed by every vector format. Rather than
degrade them silently, the exporters that cannot represent an effect throw a typed
`UnsupportedOperationError` naming the operation and renderer:

| Effect | Bitmap | Canvas | SVG | PDF | PostScript |
| --- | --- | --- | --- | --- | --- |
| Conic / angular gradient | renders | renders | unsupported | unsupported | unsupported |
| Explicit drop shadow (`drawShadow(of:)`) | renders | renders | renders | unsupported | unsupported |
| Projective (perspective) image map | renders | unsupported | unsupported | unsupported | unsupported |

The reasons are documented at each throw site: SVG/PDF/PostScript shading has no conic type; PDF and
PostScript have no Gaussian blur for a soft shadow; and a perspective image warp needs a patch mesh
that the affine-only vector formats cannot carry.
