...
Canonical dependency-injection principles for PureDraw and the wider PureDraw codebase: no singletons, constructor injection, protocol seams.
...
If and when PureDraw grows into multiple SPM packages:
...
If and when PureDraw is split into multiple packages, each package should lift out of the repo without modification:
...
If PureDraw ships multiple binaries, the **executable target name** stays normal (no `Impl` suffix). The binary's external identity (filename on disk, `which` output) stays clean.
