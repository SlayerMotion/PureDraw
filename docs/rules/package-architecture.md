...
How to decompose PureDraw into focused SPM targets within its single package. PureDraw is a monorepo from day one: one `Package.swift` under `Packages/`, many single-responsibility targets in it. The `PureDraw` library and the `puredraw` executable are already two such targets, so the decomposition rules below apply now and guide every target that joins the manifest.
...
PureDraw already has more than one target (the `PureDraw` library and the `puredraw` CLI). Add a new target to the single package when one of these is true:

- A part of PureDraw has become a clearly separable responsibility (a parser, a transport, a renderer) used in more than one place.
...
For PureDraw specifically, keep the `PureDraw` library name as the public anchor and grow new packages around it (for example `TileCore`, `TileParser`) rather than renaming it.
...
As PureDraw grows targets within its single package, the layout might look like this. Your actual target list will differ.
...
│
├── Domain Layer
│   └── PureDraw           # the main library; depends: TileCore, TileParser
│
└── Front-door (not under Sources/)
    └── puredraw         # the CLI executable; wires the concretes
```
