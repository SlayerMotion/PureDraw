...
The rule for what each SPM target may `import`. PureDraw is a monorepo from day one with many targets in one package, and the engine (`PureDraw`) and CLI (`puredraw`) are already two targets, so this contract applies now and governs every target added to the manifest.
...
2. Is the target an `executableTarget` / composition root (for example the `puredraw` CLI)? Allowed; the composition root wires the universe.
