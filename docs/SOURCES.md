...
| **W3C Compositing and Blending Level 1** (W3C Candidate Recommendation) | The separable blend-function formulas (`screen`, `overlay`, `darken`, `lighten`, `colorDodge`, `colorBurn`, `hardLight`, `softLight`, `difference`, `exclusion`) and the premultiplied composite that folds them in; the additive `plus-lighter`/`plus-darker` follow Apple's `CGBlendMode`. The `CGBlendMode` constants follow this model, so the software path matches `CoreGraphicsRenderer`. | `Renderers/BitmapRenderer.swift` (`blendPixel`) |
