...
struct JPEGDecoderTests {
    ...
    @Test(arguments: [
        ...
        // Progressive SOF2 declaring 65535x65535 with a valid DC scan: the coefficient store must be
        // rejected by the dimension cap, not allocated (a few bytes must not request ~100 GB).
        [
            0xFF, 0xD8, 0xFF, 0xC2, 0x00, 0x11, 0x08, 0xFF, 0xFF, 0xFF, 0xFF, 0x03,
            0x01, 0x11, 0x00, 0x02, 0x11, 0x01, 0x03, 0x11, 0x01,
            0xFF, 0xDA, 0x00, 0x0C, 0x03, 0x01, 0x00, 0x02, 0x11, 0x03, 0x11, 0x00, 0x00, 0x00, 0xFF, 0xD9,
        ],
        // Progressive DC band (Ss == 0) with Se == 63: must be rejected, not decoded as DC ignoring Se.
        [
            0xFF, 0xD8, 0xFF, 0xC2, 0x00, 0x0B, 0x08, 0x00, 0x08, 0x00, 0x08, 0x01, 0x01, 0x11, 0x00,
            0xFF, 0xDA, 0x00, 0x08, 0x01, 0x01, 0x00, 0x00, 0x3F, 0x00, 0xFF, 0xD9,
        ],
        // Progressive SOS declaring zero scan components: must throw, not index an empty array.
        [
            0xFF, 0xD8, 0xFF, 0xC2, 0x00, 0x0B, 0x08, 0x00, 0x08, 0x00, 0x08, 0x01, 0x01, 0x11, 0x00,
            0xFF, 0xDA, 0x00, 0x06, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xD9,
        ],
    ] as [[UInt8]])
