...
struct GeometryValidationTests {
    @Test func matrixInvertibilityValidation() {
        // Valid matrix
        let validTransform = Geometry.AffineTransform.scale(x: 2, y: 2)
        ...
        // Singular matrix
        let singularTransform = Geometry.AffineTransform.scale(x: 0, y: 0)
    ...
    @Test func validatorBuilderAppliesRules() {
        let t = Geometry.AffineTransform.scale(x: 0, y: 0)
    ...
    @Test func matrixFinitenessValidation() {
        let infiniteMatrix = Geometry.AffineTransform(a: 1, b: .infinity, c: 0, d: 1, tx: 0, ty: 0)
        do {
