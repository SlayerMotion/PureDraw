...
typealias AffineTransform = PureGeometry.AffineTransform
...
struct AffineTransformTests {
    ...
    private func isAlmostEqual(_ t1: AffineTransform, _ t2: AffineTransform) -> Bool {
        isAlmostEqual(t1.a, t2.a) &&
    ...
    @Test func singularMatrixInversion() {
        ...
        let singular = AffineTransform.scale(x: 0, y: 0)
    ...
    @Test func pointApplication() {
        ...
        // Test basic scale
        let t1 = AffineTransform.scale(x: 2, y: 3)
        let r1 = p.applying(t1)
        #expect(r1.x == 20 && r1.y == 30)

        // Test basic translation
        let t2 = AffineTransform.translation(x: 5, y: -5)
    ...
    @Test func skewTransformation() {
        let p = Point(x: 10, y: 10)
        let t = AffineTransform.skew(x: 0.5, y: 0.0)
