    ...
    public func render(_ context: GraphicsContext) throws -> Data {
        ...
        for op in context.commands {
            ...
            if fillAlpha < 1.0 || strokeAlpha < 1.0 {
                let key = ExtGStateKey(ca: fillAlpha, CA: strokeAlpha)
                let gsName: String
        ...
        if !extGStates.isEmpty {
            ...
            for (key, gsName) in extGStates {
                resourcesStr += "\n    /\(gsName) << /Type /ExtGState /ca \(key.ca) /CA \(key.CA) >>"
            }
            resourcesStr += "\n  >>"
        }
        if !shadings.isEmpty {
    ...
    private class PDFWriter {
        ...
        func buildData() -> Data {
            ...
            for obj in objects {
                ...
                let objData = obj.data(using: .utf8)!
                data.append(objData)
            ...
            %%EOF
            """
            data.append(trailer.data(using: .utf8)!)
